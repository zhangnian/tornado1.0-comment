...
class IOLoop(object):
    ...
    def set_blocking_log_threshold(self, s):
        ...
        if not hasattr(signal, "setitimer"):
            logging.error(
                "set_blocking_log_threshold requires a signal module "
    ...
    def start(self):
        ...
        while True:
            ...
            if self._timeouts:
                now = time.time()
                # 遍历执行所有已超时的timeout callback
                while self._timeouts and self._timeouts[0].deadline <= now:
                    timeout = self._timeouts.pop(0)
                    self._run_callback(timeout.callback)

                # 如果还剩下未超时的timeout callback
                if self._timeouts:
                    # 计算出最快一次超时的时间间隔
    ...
    def _wake(self):
        try:
            ...
            # 之前已经向poller注册了管道读端的可读事件，通过向管道的写端写入一个字节，从而让poll()返回
            # 这个机制用于作为结束事件循环的通知
